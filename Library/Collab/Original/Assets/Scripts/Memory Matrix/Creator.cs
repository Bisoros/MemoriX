...
public class Creator : MonoBehaviour
{
    ...
    private int lines, columns, lenMargin = 1, lenCube = 2, low = -1, high = 2, modifier, rotationSpeed=100, direction, dif;
    ...
    private void Start()
    {
        ...
        //columns = 2 + (int)Mathf.Round(1.5f*Mathf.Log(PlayerPrefs.GetInt("difficulty"))) + Random.Range(low, high);
        //if (PlayerPrefs.GetInt("difficulty") == 0)
        //columns = 2 + Random.Range(low, high);

        columns = 2 + (int)Mathf.Round(3 * Mathf.Pow(PlayerPrefs.GetInt("difficulty"), .075f));

        lines = columns + Random.Range(low, high);
